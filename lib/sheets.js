...
async function setupSpreadsheet(spreadsheetId, tabSpecs) {
  ...
  // 헤더가 비어 있으면 헤더+예시행을 채우고, 헤더 문구가 코드와 다르면 1행만 최신 문구로 맞춘다
  // (열 순서는 코드가 정하므로 1행만 바꿔도 기존 데이터에는 영향이 없다)
  ...
  summary.headersUpdated = [];

  for (const spec of tabSpecs) {
    const head = await sheets.spreadsheets.values.get({ spreadsheetId, range: q(spec.title, '1:1') });
    const current = ((head.data.values || [])[0] || []).map((v) => String(v).trim());
    const hasHeader = current.some((v) => v);
    if (hasHeader) {
      const same = spec.headers.every((h, i) => (current[i] || '') === h);
      if (same) {
        ...
        summary.headersUpdated.push(spec.title);
      }
      continue;
    }
